...
def make_event(
    ...
    return PropertyChangedEvent(
        ...
        eventData=PropertyChangedEventData(
            prop_id, change_type, value, seq_idx)
    )
...
class NcMember:
    ...
    def get_property(self, oid: int, id_args: IdArgs): ...
    def set_property(self, oid: int, id_args_value: IdArgsValue): ...
    def invoke_method(self, oid: int, method_id: ElementId, args: Any): ...
...
class NcObject(NcMember):
    ...
    def member_type(self): return "NcObject"
    def get_role(self): return self.role
    def get_oid(self): return self.oid
    def get_constant_oid(self): return self.constant_oid
    def get_class_id(self): return self.class_id
    def get_user_label(self): return self.user_label
    ...
    async def _notify(self, prop_id, change_type, value, seq_idx=None):
        await self.notifier.put(make_event(self.oid, prop_id, change_type, value, seq_idx))

    def set_property(self, oid, id_args_value):
        if id_args_value.id.level == 1 and id_args_value.id.index == 6 and isinstance(id_args_value.value, str):
            self.user_label = id_args_value.value
            asyncio.create_task(self._notify(id_args_value.id, NcPropertyChangeType.ValueChanged, id_args_value.value))
            return None, True
...
class NcBlock(NcMember):
    def __init__(self, is_root, class_id, oid, constant_oid, owner, role, user_label, enabled, notifier):
        self.base = NcObject(class_id, oid, constant_oid, owner, role, user_label, notifier)
        self.is_root = is_root
        self.enabled = enabled
        self.members: List[NcMember] = []

    def member_type(self): return "NcBlock"
    def get_role(self): return self.base.get_role()
    def get_oid(self): return self.base.get_oid()
    def get_constant_oid(self): return self.base.get_constant_oid()
    def get_class_id(self): return self.base.get_class_id()
    def get_user_label(self): return self.base.get_user_label()

    def get_property(self, oid, id_args):
        if oid == self.base.oid:
            lvl, idx = id_args.id.level, id_args.id.index
            if (lvl,idx) == (2,1): return None, self.enabled
            if (lvl,idx) == (2,2): return None, self.generate_members_descriptors()
    ...
    def set_property(self, oid, id_args_value):
        if oid == self.base.oid: return self.base.set_property(oid,id_args_value)
        m = self.find_member(oid)
        return m.set_property(oid,id_args_value) if m else ("Member not found", False)

    def invoke_method(self, oid, method_id, args):
        if oid == self.base.oid: return self.base.invoke_method(oid, method_id, args)
        m = self.find_member(oid)
        return m.invoke_method(oid, method_id, args) if m else ("Member not found", None)

    def add_member(self, member):
        self.members.append(member)
        ev = make_event(self.base.oid, ElementId(2,2), NcPropertyChangeType.ValueChanged, self.generate_members_descriptors())
        asyncio.create_task(self.base.notifier.put(ev))

    def find_member(self, oid):
        for m in self.members:
            if m.get_oid() == oid: return m
    ...
    def generate_members_descriptors(self):
        return [{"role": m.get_role(),
                 "oid": m.get_oid(),
                 "constantOid": m.get_constant_oid(),
                 "classId": m.get_class_id(),
                 "userLabel": m.get_user_label() or "",
                 "owner": self.base.get_oid()} for m in self.members]

# --- App state ---


class ConnectionState:
    def __init__(self, ws): self.websocket, self.subscribed_oids = ws, set()
...
class AppState:
    def __init__(self):
        ...
        self.device = NmosDevice(
            id=str(uuid.uuid4()), label="Example Device", description="NMOS Example Device",
            senders=[], receivers=[], node_id=str(uuid.uuid4()),
            type="urn:x-nmos:device:generic", version=tai_timestamp(),
            controls=[DeviceControl("urn:x-nmos:control:ncp/v1.0", "ws://127.0.0.1:3000/ws", False)]
        )

    async def notify_subscribers(self, ev):
        text = json.dumps({"messageType": MESSAGE_TYPE_NOTIFICATION, "notifications":[serialize_event(ev)]}, cls=CustomEncoder)
        for conn in list(self.connections.values()):
            if ev.oid in conn.subscribed_oids:
                try: await conn.send_text(text)
                except: pass

    async def event_loop(self):
        while True: await self.notify_subscribers(await self.event_queue.get())
...
async def devices_handler(request): return web.json_response([asdict(app_state.device)])


async def device_handler(request):
    device_id = request.match_info.get("device_id")
    if device_id == app_state.device.id: return web.json_response(asdict(app_state.device))
    ...
    for cmd in msg.get("commands", []):
        ...
        try:
            if (method_id.level, method_id.index) == (1,1):
                ...
                if err: status, error = 400, err
            elif (method_id.level, method_id.index) == (1,2):
                err, ok = root.set_property(oid, IdArgsValue(ElementId(**args["id"]), args.get("value")))
                if not ok: status, error = 400, err
            else:
                err, resp = root.invoke_method(oid, method_id, args)
                if err is None: value = resp
                else: status, error = 400, err
        except Exception as e: status, error = 400, str(e)

        responses.append({"handle":handle, "result":{"status":status,"value":value,"errorMessage":error}})
...
async def websocket_handler(request):
    ...
    try:
        async for msg in ws:
            ...
            except: await conn.send_text(json.dumps({"messageType":MESSAGE_TYPE_ERROR,"status":400,"errorMessage":"Invalid JSON"})); continue

            mt = data.get("messageType")
            if mt == MESSAGE_TYPE_COMMAND and "commands" in data:
                await conn.send_text(json.dumps(await process_command(data), cls=CustomEncoder))
            elif mt == MESSAGE_TYPE_SUBSCRIPTION and "subscriptions" in data:
                conn.subscribed_oids = set(data["subscriptions"])
                await conn.send_text(json.dumps({"messageType":MESSAGE_TYPE_SUBSCRIPTION_RESPONSE,"subscriptions":list(conn.subscribed_oids)}))
            else:
                await conn.send_text(json.dumps({"messageType":MESSAGE_TYPE_ERROR,"status":400,"errorMessage":"Invalid message"}))
    finally:
...
async def init_app():
    ...
    # Root block
    root = NcBlock(True, [1, 1], 1, True, None, "root", None, True, app_state.event_queue)
    # Child member
    root.add_member(NcObject([1], 2, True, 1, "test", "test", app_state.event_queue))
    # Child block
    child_block = NcBlock(False, [1,1], 3, True, None, "child_block", None, True, app_state.event_queue)
    child_block.add_member(NcObject([1], 4, True, 3, "child_block_member", "Child",app_state.event_queue))
